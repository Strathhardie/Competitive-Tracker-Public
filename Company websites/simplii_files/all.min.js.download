...
}).on("close.fndtn.reveal","[data-reveal]",function(){$("body").css("overflow","auto")
});
...
}(function(){$(function(){var a=navigator.userAgent||navigator.vendor||window.opera;
if(/windows phone/i.test(a)){$("body").addClass("windows-mobile")
}else{if(/android/i.test(a)){$("body").addClass("android")
}else{if(/iPad|iPhone|iPod/.test(a)&&!window.MSStream){$("body").addClass("ios")
}else{if(/BB10|BlackBerry/i.test(a)){$("body").addClass("blackberry")
}}}}})
})();
...
(function(f){$(".slickslider").slick();
$(f).resize(function(){$(".slickslider").slick("slickGoTo",0)
});
$(".slickslider").each(function(){if($(this).hasClass("equalizechildcomponents")){var h=$(this).data("equalizer");
$(this).find(".slickslide-inner").each(function(){var i=$(this).children().first();
if(i.hasClass("featurebannercontainer")||i.find(".featurebannercontainer").length||i.find(".featurebannerreference").length||i.find("adrotator").length){$(".banner",i).each(function(j,k){$(k).attr("data-equalizer-watch",h);
$(k).hasClass("flexible-padding")||$(k).addClass("flexible-padding");
$(k).find(".row[data-equalizer]").removeAttr("data-equalizer").removeAttr("data-equalizer-mq");
$(".columns[data-equalizer-watch]",k).attr("data-equalizer-watch",h)
})
}else{if(i.find(".callout").length||i.hasClass("callout")){$(".callout-inner",i).attr("data-equalizer-watch",h)
}else{if(i.hasClass("reference")){$(".cq-dd-paragraph > div > div",i).attr("data-equalizer-watch",h)
}else{i.children().first().attr("data-equalizer-watch",h)
}}}})
}$(this).on("setPosition",function(i){if($(i.target).find(".slick-active").length===$(i.target).find(".slick-slide").length){$(i.target).parent().prev(".carouselhelp").css("display","none");
$(i.target).find(".slickslide > .show-for-sr").css("display","none")
}else{$(i.target).parent().prev(".carouselhelp").css("display","block");
$(i.target).find(".slickslide > .show-for-sr").css("display","block")
}})
});
"use strict";
if(!$){$=f.jQuery
}var d=$('.carousel[data-comp="carousel"]'),c=false;
d.slick({dots:true,arrows:false,fade:!!d.hasClass("fade"),autoplay:true,autoplaySpeed:7000,speed:250});
$(d).on("click",function(h){if($(".slick-dots",this).find(h.target).length>0&&this.slick.slickGetOption("autoplay")){this.slick.slickSetOption("autoplay",!1)
}});
$(".carousel .slick-list").removeAttr("aria-live");
$(d).focusout(function(h){$(".carousel .slick-list").removeAttr("aria-live")
});
$(d).focusin(function(h){$(".carousel .slick-list").attr("aria-live","polite")
});
$(d).keypress(function(h){if(h.which===32){h.preventDefault();
if(this.slick.paused){this.slick.play()
}else{this.slick.pause()
}}});
var a=$(".carousel.inherit").parent().parent().parent().parent().parent(".full-bleed");
if(a.length>0){a.addClass("carousel-target");
$(document).on("replace",".banner",function(l,i){var j=$(".transition.current",a),h=$(".transition.next",a),k;
j=(j.length<=0)?$('<div class="transition current">'):j;
h=(h.length<=0)?$('<div class="transition next">'):h;
$(".banner",d).each(function(){var m=$(this);
if(m.css("background-image")!="none"){m.data("background-image",m.css("background-image"))
}if(m.css("background-color")!="transparent"&&m.css("background-color")!="rgba(0, 0, 0, 0)"){m.data("background-color",m.css("background-color"))
}m.css({"background-color":"transparent","background-image":"none"})
});
if(!c){k={position:"absolute",width:"100%",height:"100%",top:0,left:0,"z-index":1,opacity:0,"background-size":"cover","background-repeat":"no-repeat","background-position":"center"};
j.css(k).prependTo(a);
h.css(k).prependTo(a);
c=true
}j.css({"background-image":($(".slick-current .banner",d).data("background-image")),"background-color":($(".slick-current .banner",d).data("background-color")),opacity:1});
h.css({opacity:0})
});
if($("html").hasClass("lt-ie9")){var e=$('<div class="transition current">'),b=$('<div class="transition next">'),g={position:"absolute",width:"100%",height:"100%",top:0,left:0,"z-index":1,opacity:0,"background-size":"cover","background-repeat":"no-repeat","background-position":"center"};
$(".banner",d).each(function(){var i=$(this),h=/\[?([a-zA-Z\.\/\-\d]+), ?\(large\)\]/i,j=h.exec(i.attr("data-interchange"));
i.data("background-image",j?"url("+j[1]+")":"");
i.data("background-color",i.css("background-color"));
i.css({"background-color":"transparent","background-image":"none"})
});
e.css(g).prependTo(a);
b.css(g).prependTo(a);
e.css({"background-image":($(".slick-current .banner",d).data("background-image")),"background-color":($(".slick-current .banner",d).data("background-color")),opacity:1});
b.css({opacity:0})
}d.on("beforeChange",function(m,i,o,n){var k=$(".banner",i.$slides[o]),j=$(".banner",i.$slides[n]),l=$(".transition.current",a),h=$(".transition.next",a);
l.css({"background-color":k.data("background-color"),"background-image":k.data("background-image"),opacity:1,"z-index":2});
h.css({"background-color":j.data("background-color"),"background-image":j.data("background-image"),opacity:0,"z-index":1});
l.animate({opacity:0},250,function(){$(this).css({"z-index":1})
});
h.animate({opacity:1},250,function(){$(this).css({"z-index":2})
})
})
}$(function(){if($(".carousel").length>0){$(f).on("resize",Foundation.utils.throttle(function(){$(document).foundation("equalizer","reflow")
},300))
}})
})(window);
...
}a.services.RDS.onReady({success:function(e){window.services.RDS.updatePageRates()
}})
});
...
(function(){$(function(){var b=$('nav[data-comp="stickynav"]'),h=b.filter(".large-up")[0],j=b.filter(".mobile-section-nav")[0],k=$(".sticky-nav-items ul a",h),g=$(".sticky-nav-items ul a",j),m=$.map(k,function(n){return $(""+n.getAttribute("href"))
}),f=$(".mobile-menu-cont"),a=$(".sticky-nav-anchor",j),e=$(".sticky-nav-anchor",j).text(),d=$(".breadcrumbs > li:last > a").text()||e,c=12;
function i(){if(h){new Waypoint.Sticky({element:h,stuckClass:"pinned"})
}}i();
$(m).waypoint(function(r){var t,o,n=$('a[href="'+this.element.selector+'"]',h),q=$('a[href="'+this.element.selector+'"]',j);
if(this.previous()&&r==="up"){var p=this.previous().previous()&&this.previous().key=="waypoint-0"?this.previous().previous():this.previous();
n=$('a[href="'+p.element.selector+'"]',h);
q=$('a[href="'+p.element.selector+'"]',j)
}t=n.parent();
...
o.addClass("active");
if(r==="up"&&this.key=="waypoint-1"){o.removeClass("active");
a.attr("title",d).attr("aria-label",d).text(d)
}else{a.attr("title",q.text()).attr("aria-label",q.text()).text(q.clone().children().remove().end().text())
}},{offset:(function(){return($(h).height()>0?$(h).height()+c:a.outerHeight()+c)
})()});
$(window).on("resize",Foundation.utils.debounce(function(){if(h){$(h).removeClass("pinned").closest("div.sticky-wrapper").css("height","");
i()
}},200));
function l(){$(".mobile-section-nav > .wrapper").css({"max-height":0});
...
}$(window).on("resize",function(n){l()
});
k.on("click",function(n){$.scrollTo($(this).attr("href"),{duration:1000,offset:{left:0,top:-($(h).height()+c-5)}});
return false
});
g.on("click",function(n){$.scrollTo($(this).attr("href"),{duration:1000,offset:{left:"-10px",top:-(a.outerHeight()+c-5)}});
...
}}$(document).on("click",'.terms-content-wrapper a[href^="#"]',function(){var r=$($(this).attr("href"));
var q=r.position();
$("html, body").animate({scrollTop:(q.top-$("header section.header-top-bar").height())},"slow")
});
$(function(){var y=$(".terms-navigation > a"),q=$(".terms-content-wrapper .content"),x=D(),C=$(".sticky-nav"),F=$(".mobile-menu-cont"),A={},w=0,G=0,E=0,B=[],z=[],v=[],r=document.documentElement.getAttribute("lang"),u=$("a.termsandconditions-footnote-link").hide();
function D(){x=q.find("a[name], a[id]").not("footnote-unmarked");
if(y[0]&&x[0]){x.each(function(){if($(this).attr("name")){g.push($(this).attr("name"))
}else{g.push($(this).attr("id"))
}})
}return x
}if(C[0]){b+=C.outerHeight()
}if(F[0]){b+=F.outerHeight()
}y.click(function(M){M.preventDefault();
var I=$(this).parent(),N=$(this).parents(".terms-wrapper")[0],L=$(this).find(".termsArrow"),J=$(".terms-content-inner-wrapper > .content"),H=$(".terms-header-wrapper",N),K=$('a[name="'+i+'"]').length?$('a[name="'+i+'"]'):$('a[id="'+i+'"]');
i=window.location.hash.replace("#",f);
if(i.slice(0,2)===d){K=$('dd[id="'+i+'"]')
}H.toggleClass("tc-close tc-open");
if(J.css("display")=="none"){$(this).addClass("active");
L.removeClass("icon-arrow-down");
L.addClass("icon-arrow-up");
J.slideDown(300,function(){y.parent().addClass("expanded").removeClass("collapsed");
if(x[0]&&($.inArray(i.replace(d,f),g)!==-1)){var P=$(K).offset().top-b;
$("html, body").animate({scrollTop:P},400,function(){if(j.indexOf("-rf")>=0){var R=j.slice(8,j.indexOf(".html")+5),S="#"+j.slice(j.indexOf(".html")+5,j.length),Q=S.slice(0,S.indexOf("-")),T=R+S;
$(Q).find('a[href*="fn"]').attr("href",T);
if(Q=="#termsandconditions"||Q=="#modalites"){$(Q).attr("href",T)
}window.history.replaceState(null,null,l.slice(0,l.lastIndexOf("?")))
}p();
K.attr("tabindex","-1");
K[0].focus()
})
}else{var O=$(this).offset().top-120;
y.parent().addClass("expanded").removeClass("collapsed");
$("body,html").animate({scrollTop:O})
}});
I.attr("aria-selected","true");
I.attr("aria-expanded","true")
}else{I.attr("aria-selected","false");
I.attr("aria-expanded","false");
J.slideUp(350,function(){var O=$(this).offset().top;
y.parent().removeClass("expanded").addClass("collapsed");
$("body,html").animate({scrollTop:O})
});
$(this).removeClass("active")
}});
$(window).load(function(){var M=$(".terms-header-wrapper.tc-open").length?true:false,K=[],J=$('a[href*="#"]').length,L=$('<script src="/etc/designs/cibcpublic/clientlibs/assets/footnotes/wet-boew.min.js"/>'),I=$('<script src="/etc/designs/cibcpublic/clientlibs/assets/footnotes/footnotes.js"/>');
$('a[href*="#"]').each(function(P){var S=$(this).attr("href"),T=S.slice(S.lastIndexOf("#")+1),O=S.slice(0,S.lastIndexOf("#")),N,R;
if((T.slice(0,2)===d||T.indexOf("termsandconditions")>=0||T.indexOf("modalites")>=0)&&T.indexOf("-")===-1){$(this).addClass("fn-lnk");
if(!(O===f||O===n)){$(this).addClass("df-ref")
}else{$(this).attr("href",S.slice(S.lastIndexOf("#")))
}K.push(T);
if($(this).children("sup")[0]){N=$(this).children("sup");
N.contents().unwrap();
$(this).wrap(N)
}R=$(this).parents("sup");
R.attr("id",T+"-"+k(K,T)+"-rf");
if(T.indexOf("termsandconditions")>=0||T.indexOf("modalites")>=0){$(this).parent().attr("id",T+"-"+k(K,T)+"-rf")
}}if($(this).hasClass("df-ref")){var Q=R[0]?R.prop("id"):$(this).parent().prop("id");
if(fn_query.indexOf("cards")===-1){fn_query=f
}$(this).attr("href",O+"?fn-lnk="+n+Q+"#"+T)
}});
$("a.fn-lnk").click(function(){var O=$(this).attr("href"),P="#"+$(this).parents("sup").attr("id"),Q=O.slice(O.lastIndexOf("#")+1),N="#"+Q.replace(d,f);
$(N).attr("href",P);
if($(".terms-content-inner-wrapper > .content").not(":visible")[0]){window.location.hash=Q;
y.trigger("click")
}});
if($(".terms-navigation")[0]){D().each(function(){if($(this).prop("href")===f){if($(this).attr("name")&&!$(this).prop("id")){var N=$(this).attr("name");
$(this).removeAttr("name");
$(this).prop("id",N)
}var Q=$(this).attr("name")?$(this).attr("name"):$(this).attr("id"),O=$("<dd/>",{id:d+Q,tabindex:"-1"}),P=$(this).closest("p"),T=$(".terms-wrapper").data("link-back-text")?$(".terms-wrapper").data("link-back-text"):f,S=$("<dt/>",{text:$(".termsTitle").text()+" "+Q});
var R="#fn"+Q+"-1-rf";
if(!($(this).prop("id")==="termsandconditions"||$(this).prop("id")==="modalites")){P.wrap(O);
$(this).closest("dd").before(S);
$(this).detach();
P.after($(this));
$(this).attr("href",R);
if(Q.startsWith(c)){Q=a[parseInt(Q.charAt(2))-1].code
}$(this).html(Q);
$(this).prepend('<span class="wb-inv">'+T+" </span>");
$(this).wrap('<p class="fn-rtn" />').wrap("<sup></sup>")
}else{if((window.location.hash.indexOf("termsandconditions")>=0||window.location.hash.indexOf("modalites")>=0)||$('[href="#termsandconditions"]')[0]||$('[href="#modalites"]')[0]){$(this).append('<span class="icon icon-arrow-left"></span><span class="wb-inv">'+T+Q+" </span>");
$(this).attr("href",R);
$(this).wrap('<p class="fn-rtn" />').wrap("<sup></sup>")
}}}});
$("span.icon > i.removal-marker").remove();
if(y[0]&&x[0]&&($.inArray(i.replace(d,f),g)!==-1)){if(!M){y.trigger("click")
...
$("html, body").animate({scrollTop:$(H).offset().top-b},400,function(){p();
$(H).attr("tabindex","-1");
$(H)[0].focus()
})
}});
...
u.each(function(L,K){var M=$(this),H=M.parent(".termsandconditions-footnote"),I="/jcr:content/data/master.json",P=H.data("fragmentRef")+I,J=H.data("footnoteType"),O,N;
if(A[P]){O=A[P]
}else{if(J=="numeric"){w+=1;
O=d+w;
A[P]=O
}else{if(J=="symbolic"){G+=1;
O=e+G;
A[P]=O
}else{E+=1;
O=m+E;
...
if(J=="numeric"){M.text(O.replace(d,f));
M.show()
}else{if(J=="symbolic"){M.html(a[parseInt(O.replace(e,""))-1].code);
M.attr(o,a[G-1][o+r]);
M.show()
}else{if(M.parents("aside.wb-fnote").length){M.closest("p").remove()
}}}});
if(!$.isEmptyObject(A)){$.when.apply($,Object.keys(A).map(function(H){return $.ajax(H)
})).done(function(){var L,I,H=r.charAt(0).toUpperCase()+r.substr(1),K;
if(this.constructor===Array){for(var J=0;
J<this.length;
J++){L=arguments[J][0]["text"+H];
I=this[J].url;
t(I,L)
}}else{I=this.url;
L=arguments[0]["text"+H];
t(I,L)
}q.children().prepend(B);
q.children().prepend(z);
q.children().prepend(v);
$.getScript("/etc/designs/cibcpublic/clientlibs/rds/js/rds-integration.js")
})
}function t(I,K){if(K){var J=A[I],H=J.replace(d,f);
if(!H.startsWith(h)){K=K.replace("<p>","<p><a id='"+H+"'></a>");
if(H.startsWith(c)){z.push(K)
}else{B.push(K)
}}else{v.push(K)
}}}});
...
(function(a){$(function(){$(window).on("ready load resize",function(){var e;
var f=".products-compare-list";
var g=["div.product-desc","div.product-callout","div.product-name","div.product-details"];
if(f){if($(window).width()>767){d()
}}function d(){$(document).on("after-height-change.fndtn.equalizer",function(){i([{container:f,eqTarget:g}])
})
}function c(k){var n=document.querySelectorAll(k);
var m,j;
for(var l=0;
l<n.length;
l++){m=$(n[l]).children();
j="large-"+Math.ceil(12/m.length);
m.addClass(j)
}}function i(n){var l;
for(var k=0;
k<n.length;
...
c(n[k].container);
for(var m=0;
m<n[k].eqTarget.length;
m++){l.push(n[k].container+" "+n[k].eqTarget[m]);
h(n[k].container,n[k].eqTarget[m])
}$(l.join(",")).addClass("eq-height")
}}function h(j,m){var n=document.querySelectorAll(j);
var o,l=false;
for(var k=0;
k<n.length;
k++){o=$(n[k]);
if(o.closest("li.accordion-navigation").length!=0&&!o.closest(".content").is(":visible")){l=true;
o.closest(".content").show()
}b(n[k],m);
if(l){o.closest(".content").hide();
l=false
}}}function b(j,m){var k=j.querySelectorAll(m);
var o=[];
var n;
for(var l=0;
l<k.length;
l++){k[l].style.height="auto";
o[l]=k[l].offsetHeight
}n=Math.max.apply(Math,o);
for(var l=0;
l<k.length;
l++){k[l].style.height=n+1+"px"
}}})
}());
$(".products-compare .compare-button a.button.compare").on("click",function(f){f.preventDefault();
_compareContainer=$(this).parents(".products-compare");
var b=_compareContainer.find(".products-compare-list .p-c-v.checked").map(function(){return this.id
});
var d=b.get().join(",");
var c=_compareContainer.children(".resultPage").val()+"?"+d;
if($(this).hasClass("new-window")){window.open(c,"newWindow")
...
(function(a){$(document).ready(function(){$(a).load(function(){$(".callout-image-align-left .img-cover, .callout-image-align-right .img-cover").each(function(){$(this).find(".container-img-wrapper").css("overflow","hidden");
var f=$(".callout-image-align-left").outerHeight(),e=$(this).find(".container-img-wrapper").width(),d=$(this).find("img"),c=$(document).width(),g=0;
d.css("max-width","none");
d.css("height",f>c?c-30:f);
g=d.width()-e;
if($(this).hasClass("right-position")){if(g>0){g=(0-g);
d.css("margin-left",g+"px")
}}else{if($(this).hasClass("left-position")){if(g>0){d.css("margin-left",0)
}}else{if(g>0){g=(0-g/2);
d.css("margin-left",g+"px")
}}}});
$(".callout-image-align-above, .callout-image-align-below").each(function(){var f=$(this).find(".container-img-wrapper").outerHeight(),e=$(this).find(".container-content").innerHeight(),d=$(this).find("img"),c=0,g=0;
if($(this).hasClass("callout-image-align-below")){c=$(this).outerHeight()-e-f;
$(this).find(".container-img").css("margin-top",c+"px")
}if($(this).hasClass("fixed-height-small")||$(this).hasClass("fixed-height-medium")||$(this).hasClass("fixed-height-large")){g=d.outerHeight()-f;
if($(this).hasClass("center-position")){g=(0-g/2);
d.css("margin-top",g+"px")
}else{if($(this).hasClass("bottom-position")){g=(0-g);
d.css("margin-top",g+"px")
}}}});
$.fn.setTallestHeight=function(){var c=this;
var e=[];
c.each(function(){var f=$(this).innerHeight();
e.push(f)
});
var d=Math.max.apply(null,e);
c.each(function(){$(this).innerHeight(d);
$(this).attr("data-height",d)
})
};
function b(){$(".callout > .callout.callout-bottom-cta").each(function(){var c=$(this).parent().parent().parent();
var d=c.find(".container-content");
$(d).removeAttr("style").setTallestHeight();
if(c.find(".callout-inner.border-all").length>0){c.find(".callout-inner").filter(":not(.callout-inner.border-all)").addClass("not-featured-in-row")
}});
$(document).foundation("equalizer","reflow")
}b();
$(a).resize(b)
...
(function(){$(function(){$(".accordion-navigation > a").off().click(function(c){c.preventDefault();
var b=$(this),a=b.parent();
if(a.attr("aria-selected")=="true"){a.attr({"aria-selected":"false","aria-expanded":"false"})
}else{a.attr({"aria-selected":"true","aria-expanded":"true"})
}if(!a.hasClass("expanded")){a.parent().find(".content").slideUp(300).end().find(".accordionTitle").removeClass("active").end().find("li").attr({"aria-selected":"false","aria-expanded":"false"}).removeClass("expanded");
b.addClass("active");
a.attr({"aria-selected":"true","aria-expanded":"true"}).find(".content").slideDown(350,function(){var d=b.offset().top-78;
$(this).show().parent().addClass("expanded").removeClass("collapsed");
$("body,html").animate({scrollTop:d})
})
}else{a.find(".content").slideUp(350,function(){$(this).hide().parent().removeClass("expanded").addClass("collapsed");
var d=b.offset().top-78;
$("body,html").animate({scrollTop:d})
...
function c(e){var g=$(e),d=g.find("a.tabs-anchor").data("target"),f=g.closest(".tabsContainer").find("div.content");
f.hide().eq(d).show(0,function(){if(b>a&&f.eq(d).find("[data-equalizer]")){$(document).foundation("equalizer","reflow")
}});
if(b>a){g.parent().children().removeClass("current").find("a.tabs-anchor").attr({"aria-expanded":"false","aria-selected":"false"});
g.addClass("current").find("a.tabs-anchor").attr({"aria-expanded":"true","aria-selected":"true"})
}else{var h=g.closest(".tabsContainer").find("ul.accordion li.accordion-navigation");
h.attr({"aria-expanded":"false","aria-selected":"false"}).find("a.accordionTitle.active").removeClass("active").find("span.accordionArrow").removeClass("icon-arrow-up").addClass("icon-arrow-down");
h.eq(d).attr({"aria-expanded":"true","aria-selected":"true"}).find("a.accordionTitle").addClass("active").find("span.accordionArrow").removeClass("icon-arrow-down").addClass("icon-arrow-up")
}}$(".tabsNavigator li").on("click",function(g){c(this);
var d=$(this).parent("ul.tabsNavigator");
if(d.data("options")=="deep_linking:true"){var f=$(this).find("a.tabs-anchor").attr("href");
if(b>a&&f!="#"){$("body").animate({scrollTop:$(f).offset().top-d.outerHeight()-75},400);
window.location.hash=f
}}return false
});
$(".tabsContainer").each(function(){var e=this.querySelector("ul.tabsNavigator li");
if($(this).find("ul.tabsNavigator").data("options")=="deep_linking:true"){$(document).foundation({tab:{deep_linking:true}});
if(b<a){$(this).find("a.accordionTitle").on("click touchstart",function(h){var g=$(this).attr("href");
if(g!="#"){window.location.hash=g
}})
}var f=window.location.hash,d;
if(window.location.hash&&$('a.tabs-anchor[href="'+f+'"]').length){if(b>a){d=$('a.tabs-anchor[href="'+f+'"]')
}else{d=$('a.accordionTitle[href="'+f+'"]')
}if(!$(this).is(".tabs-closed")){d.trigger("click")
}}else{if(!$(this).is(".tabs-closed")){c(e)
}}}else{if(!$(this).is(".tabs-closed")){c(e)
}}})
})
})();
...
$.ajax({url:f,async:true,cache:false,type:"POST",dataType:"json",data:JSON.stringify(n,null," "),headers:{accept:"application/vnd.api+json",brand:"simplii","content-type":"application/vnd.api+json"},success:function(){window.location="https://www.simplii.com/ca/"+o
},error:function(){window.location="https://www.simplii.com/ca/apply/afp-error"+p+".html"
}})
...
function secureCheck(){if(location.host.indexOf("simplii.com")!=-1&&location.host!="www.simplii.com"){location.href=location.href.replace("simplii.com","www.simplii.com")
}if(location.host=="www.simplii.com"&&location.protocol!="https:"){location.href="https:"+location.href.replace(location.protocol,"")
...
}}}},error:function(c){if(b.delegate&&b.delegate.error){b.delegate.error({response:c,code:"001"},a,b)
}},complete:function(){if(b.delegate&&b.delegate.complete){b.delegate.complete(a,b)
}}})
}})(window,{url:signon_json_url,delegate:{error:function(a,b,c){if(a&&a.code){b.$(".sign-on .warning > span").append('<span class="error-code">E'+a.code+"</span>")
}b.$(".sign-on form").hide();
b.$(".sign-on .warning").show()
}},factory:{"1.0":function(d,b,e){var a=null,c=null;
...
try{this.timeout=b.setTimeout(function(){f.onError({message:"Signon API Timeout",code:"002"})
},d.loadTimeout);
b.document.domain=d.domain;
$.ajax({url:d.resources.status,cache:false,dataType:"html",success:function(h){f.onLoad()
},error:function(){f.onError({message:"AJAX Request Failed.",code:"003"})
},complete:function(){b.clearTimeout(f.timeout)
}})
}catch(g){this.onError(g)
}},onError:function(f){c.onError(f)
},onInit:function(){b.clearTimeout(this.timeout)
},onReady:function(f){c.begin(f)
},onLoad:function(g){var f=this;
b.location.origin=d.resources.ingressurl;
...
var i=k.reverse().map(function(n){return $("<option/>").attr({value:n.value}).text(n.label?n.label:"")
});
a.onReady({$cards:$("<select/>").append(i)})
}else{a.onReady({$cards:[]})
}break;
case"ui:ready":j&&j.postMessage(JSON.stringify({key:"bankCards",method:"get"}),a.olburl);
break
}})
})
}};
...
$(".remember-card > input",this).on("change",function(i){$(this).parents(".sign-on-fields").find(".save-card-description").toggle(i.target.checked)
})
}});
c.binded=true
},onError:function(f){if(e.delegate&&e.delegate.error){e.delegate.error(f,b,e)
...
a.parameters.description=$(".save-card-description input",b.$(g).closest(".sign-on-form")).val();
...
}}if(d.indexOf(c)>=0){$("#desktop-header .flex-dropdown.collapsed button.search-button").hide();
...
if(f.trim()==""){if($("html").attr("lang")=="en"){f="//sp1004f987.guided.ss-omtrdc.net"
}else{if($("html").attr("lang")=="fr"){f="//sp1004f955.guided.ss-omtrdc.net/"
}}}if(b.trim()==""){if($("html").attr("lang")=="en"){b="//content.atomz.com/autocomplete/sp10/04/f9/87/"
}else{if($("html").attr("lang")=="fr"){b="//content.atomz.com/autocomplete/sp10/04/f9/87/"
...
window.services.search.facet=$(".facets.service .facet.item.active .facet.button").data("facet");
C()
});
...
}a.services.RDS.onReady({success:function(ag){window.services.RDS.updateSandPRDSBannerAdsRates()
}})
...
}$(function(){$(".header-search-container form").each(function(){var i=$(this);
var b="#searchInputPick";
var f=i.find("input.search-input");
var h=".search-pickitem-";
var d=10;
var c=0;
var g=0;
function e(k){f.val(k.text());
i.submit()
}function a(p){i.siblings("#searchInputPick").remove();
$(".header-search-container form.search-wrapper .search-input").attr("aria-expanded","false");
$("form.search-wrapper input.search-input").attr("aria-activedescendant","");
$("form.search-wrapper input.search-input").attr("aria-label","");
var r;
var q="";
var m=$("<ul id='searchInputPick' aria-labelledby='search-label' role='listbox'></ul>");
if(p&&p.length){var n=0;
g=p.length;
for(var o in p){if(n++<d){var l=p[o];
var k=$("<li id='search-pickitem-"+n+"' role='option' class='search-pickitem search-pickitem-"+n+"' data-item='"+n+"'></li>").text(l||"");
k.click(function(){j($(this).data("item"));
e($(this))
}).hover(function(){j($(this).data("item"))
});
m.append(k);
if(d>g&&g!=0){r=g
}else{r=d
}q=r+" search suggestions available. Use up and down arrow keys to navigate.";
$(".header-search-container .id-search-suggestions").text(q);
$(".header-search-container form.search-wrapper .search-input").attr("aria-expanded","true")
}}c=0
}else{$(".header-search-container .id-search-suggestions").text("0 search suggestions available. Use up and down arrow keys to navigate.")
}i.parent().append(m.show())
}function j(k){c=parseInt(k);
var m=$(b);
var l=m.find(h+c);
if(l.length){m.find(".search-highlight").removeClass("search-highlight");
l.addClass("search-highlight")
}}i.submit(function(k){if(!f.val().length>=3){k.preventDefault()
}});
f.keyup(function(l){var n=$(this);
var m;
if(d>g&&g!=0){m=g
}else{m=d
}switch(l.which){case 13:var k=$(b).find(".search-highlight");
if(k.length){e(k)
}break;
case 27:i.siblings(b).remove();
n.val("").focus();
$(".header-content-container .header-search-container form.search-wrapper .search-input").attr("aria-expanded","false");
$("form.search-wrapper input.search-input").attr("aria-activedescendant","");
$(".header-search-container .id-search-suggestions").text("0 search suggestions available. Use up and down arrow keys to navigate.");
c=0;
break;
case 38:if(c>0){j(--c)
}$(".search-mobile .header-search-container form.search-wrapper .search-input").val($(" .search-pickitem.search-highlight").text());
$(".search-header-desktop-container-inner .header-search-container form.search-wrapper .search-input").val($(" .search-pickitem.search-highlight").text());
$(i).find(".id-search-suggestions").text($("li#search-pickitem-"+c).text()+" "+c+" of "+m);
if(c!=0){$("form.search-wrapper input.search-input").attr("aria-activedescendant","search-pickitem-"+c)
}else{$("form.search-wrapper input.search-input").attr("aria-activedescendant","")
}break;
case 40:if(c<g){j(++c)
}$(".search-mobile .header-search-container form.search-wrapper .search-input").val($(" .search-pickitem.search-highlight").text());
$(".search-header-desktop-container-inner .header-search-container form.search-wrapper .search-input").val($(" .search-pickitem.search-highlight").text());
$(i).find(".id-search-suggestions").first().text($("li#search-pickitem-"+c).text()+" "+c+" of "+m);
$("form.search-wrapper input.search-input").attr("aria-activedescendant","search-pickitem-"+c);
break;
default:if((n.val()||"").length>=3){window.services.search.autocomplete(n.val(),function(o){a(o)
})
}else{i.siblings(b).remove();
$(".header-content-container .header-search-container form.search-wrapper .search-input").attr("aria-expanded","false");
$("form.search-wrapper input.search-input").attr("aria-activedescendant","");
$(".header-search-container .id-search-suggestions").text("0 search suggestions available. Use up and down arrow keys to navigate.")
}break
...
$(".search-promote .facets-filter-mobile button.button").on("click",function(b){$(".search-promote .facets-container-mobile").toggleClass("hidden");
...
if(n=="simpliiresultsfasets"){if($(".facets.service .all .label-count").html().replace(" ","")=="(0)"){$(".promote.service").addClass("large-push-2");
$(".promote.service").removeClass("large-push-4")
}else{$(".promote.service").addClass("large-push-4");
$(".promote.service").removeClass("large-push-2")
}}b.show();
var m=0;
...
this.dispatchService=function(){for(var d in a.dispatchQueue){if(a.ready){if(a.instance.getRates){if(a.dispatchQueue[d].lob){a.instance.getRates(a.dispatchQueue[d].lob,a.dispatchQueue[d].products,a.dispatchQueue[d].delegate)
}else{if(a.dispatchQueue[d].dataset){a.instance.requestRates(a.dispatchQueue[d].dataset,a.dispatchQueue[d].delegate)
...
a.response=d;
localStorage.setItem("rdsConfigParams",JSON.stringify(d));
...
},requestRates:function(e,d){a.dispatchQueue.push({dataset:e,delegate:d});
if(a.initialized){a.dispatchService()
}return this
}}
})(window,{url:rds_url,factory:{"2.0":function(b,a,c){return c.factory["1.0"].call(this,b,a,c)
},"1.0":function(c,b,e){this.dataSet=null;
var a=this;
function d(f){var h=$.ajax({type:"GET",url:f,cache:true,async:false,dataType:"json"});
var g=h.responseText;
g=jQuery.parseJSON(g);
return g
}this.requestRates=function(m,h){var f=c.url;
if($(".data-rds[data-rds]").length>0){if(!a.dataSet){a.dataSet=d(f)
}}responseDataALL=this.dataSet;
var g,l;
for(var n in m){for(var k in m[n]){g=0;
for(var j in responseDataALL){if(responseDataALL[j].mortgageType==undefined){if(n=="3"){l=(k=="true");
if(responseDataALL[j].overdraft==l){if(m[n][k]!=null){m[n][k][g]=responseDataALL[j];
g++
}}}else{if(m[n][k]!=null){m[n][k][g]=responseDataALL[j];
g++
}}}else{if(responseDataALL[j].mortgageType==k){if(m[n][k]!=null){m[n][k][g]=responseDataALL[j];
g++
}}}}}if(h&&h.success){h.success(m,n)
}}return m
};
this.getRates=function(k,j,g){var f={};
if(typeof j!=="object"){f[j]=null
}else{for(var h in j){f[h]=null
}}var i=[];
i[k]={};
i[k]=f;
this.requestRates(i,g);
return this
};
return this
}}});
var dataSimpliiLobName=["","MORTGAGE","TREASURY","CHEQUING","SAVINGS","GIC","LOAN","RRSP","PLC","CREDIT_CARD"];
var languagePath=$("html").attr("languagePath");
var rds_url=languagePath+"/jcr:content/services.rdsdata-simplii.json";
(function(a,c){if(c){if(localStorage.getItem("rdsConfigParams")!==null){var b=JSON.parse(localStorage.getItem("rdsConfigParams"));
c.factory[b.version](b,a,c)
}}})(window,{url:rds_url,factory:{"1.0":function(data,context,configuration){configuration._data={};
...
}function getValue(rate,type){var ret=null;
var url=data.url;
...
for(var item in responseData){termArray=rate.term.split("_");
if(rate.lob=="1"&&dataSimpliiLobName[rate.lob]==responseData[item].rateProductCategory){if(responseData[item].mortgageType==rate.product){if(responseData[item].rateType==rate.rate){if(responseData[item].term.value==termArray[0]&&responseData[item].term.type==termArray[1]){if(rate.tierFrom=="undefined"){if(type=="effective"){ret=effectiveDateFormat(responseData[item].interestRates[0].effectiveDate)
}else{ret=responseData[item].interestRates[0].rateValue
}return ret
}else{tierArray=rate.tierFrom.split("-");
for(var itemSub in responseData[item].interestRates){if(responseData[item].interestRates[itemSub].tierFrom==tierArray[0]){if(type=="effective"){ret=effectiveDateFormat(responseData[item].interestRates[itemSub].effectiveDate)
}else{ret=responseData[item].interestRates[itemSub].rateValue
}return ret
}else{if(responseData[item].interestRates[itemSub].tierTo==tierArray[1]){if(type=="effective"){ret=effectiveDateFormat(responseData[item].interestRates[itemSub].effectiveDate)
}else{ret=responseData[item].interestRates[itemSub].rateValue
}}}}}}}}}else{if(rate.lob=="2"&&dataSimpliiLobName[rate.lob]==responseData[item].rateProductCategory){if(type=="effective"){ret=effectiveDateFormat(responseData[item].interestRates[0].effectiveDate)
}else{ret=responseData[item].interestRates[0].rateValue
}return ret
}else{if(rate.lob=="3"&&dataSimpliiLobName[rate.lob]==responseData[item].rateProductCategory){rateProductBoolean=(rate.product=="true");
if(responseData[item].rateType==rate.rate){if(responseData[item].overdraft==rateProductBoolean){if(responseData[item].interestRates[0].tierFrom==undefined){if(type=="effective"){ret=effectiveDateFormat(responseData[item].interestRates[0].effectiveDate)
}else{ret=responseData[item].interestRates[0].rateValue
}}else{tierArray=rate.tierFrom.split("-");
for(var itemSub in responseData[item].interestRates){if(responseData[item].interestRates[itemSub].tierFrom==tierArray[0]){if(type=="effective"){ret=effectiveDateFormat(responseData[item].interestRates[itemSub].effectiveDate)
}else{ret=responseData[item].interestRates[itemSub].rateValue
}}}}}}}else{if(rate.lob=="4"&&dataSimpliiLobName[rate.lob]==responseData[item].rateProductCategory){rateRegisteredBoolean=(rate.registered=="true");
rateInvestmentBoolean=(rate.investment=="true");
rateTaxFreeBoolean=(rate.taxFree=="true");
if(rate.registered=="undefined"&&rate.investment=="undefined"&&rate.taxFree=="undefined"){if(responseData[item].rateType==rate.rate){if(responseData[item].anniversaryBonusRates[0]!=undefined){if(responseData[item].anniversaryBonusRates[0].interestRates[0].tierFrom==undefined&&responseData[item].anniversaryBonusRates[0].interestRates[0].tierTo==undefined){if(type=="effective"){ret=effectiveDateFormat(responseData[item].anniversaryBonusRates[0].interestRates[0].effectiveDate)
}else{ret=responseData[item].anniversaryBonusRates[0].interestRates[0].rateValue
}}else{tierArray=rate.tierFrom.split("-");
for(var itemABR in responseData[item].anniversaryBonusRates){for(var itemSub in responseData[item].anniversaryBonusRates[itemABR].interestRates){if((responseData[item].anniversaryBonusRates[itemABR].interestRates[itemSub].tierFrom==tierArray[0])||(responseData[item].anniversaryBonusRates[itemABR].interestRates[itemSub].tierFrom==undefined&&"NA"==tierArray[0])){if(type=="effective"){ret=effectiveDateFormat(responseData[item].anniversaryBonusRates[itemABR].interestRates[itemSub].effectiveDate)
}else{ret=responseData[item].anniversaryBonusRates[itemABR].interestRates[itemSub].rateValue
}return ret
}}}}}}}else{if(responseData[item].rateType==rate.rate&&responseData[item].registered==rateRegisteredBoolean&&responseData[item].investment==rateInvestmentBoolean&&responseData[item].taxFree==rateTaxFreeBoolean){if(responseData[item].interestRates[0].tierFrom==undefined&&responseData[item].interestRates[0].tierTo==undefined){if(type=="effective"){ret=effectiveDateFormat(responseData[item].interestRates[0].effectiveDate)
}else{ret=responseData[item].interestRates[0].rateValue
}}else{tierArray=rate.tierFrom.split("-");
for(var itemSub in responseData[item].interestRates){if((responseData[item].interestRates[itemSub].tierFrom==tierArray[0])||(responseData[item].interestRates[itemSub].tierFrom==undefined&&"NA"==tierArray[0])){if(type=="effective"){ret=effectiveDateFormat(responseData[item].interestRates[itemSub].effectiveDate)
}else{ret=responseData[item].interestRates[itemSub].rateValue
}return ret
}}}}}}else{if(rate.lob=="5"&&dataSimpliiLobName[rate.lob]==responseData[item].rateProductCategory){rateRegisteredBoolean=(rate.registered=="true");
if(responseData[item].rateType==rate.rate&&responseData[item].registered==rateRegisteredBoolean){if(responseData[item].term.value==termArray[0]&&responseData[item].term.type==termArray[1]){if(responseData[item].interestRates[0].tierFrom==undefined&&responseData[item].interestRates[0].tierTo==undefined){if(type=="effective"){ret=effectiveDateFormat(responseData[item].interestRates[0].effectiveDate)
}else{ret=responseData[item].interestRates[0].rateValue
}}else{tierArray=rate.tierFrom.split("-");
for(var itemSub in responseData[item].interestRates){if((responseData[item].interestRates[itemSub].tierFrom==tierArray[0])||(responseData[item].interestRates[itemSub].tierFrom==undefined&&"NA"==tierArray[0])){if(type=="effective"){ret=effectiveDateFormat(responseData[item].interestRates[itemSub].effectiveDate)
}else{ret=responseData[item].interestRates[itemSub].rateValue
}return ret
}}}}}}else{if(rate.lob=="6"&&dataSimpliiLobName[rate.lob]==responseData[item].rateProductCategory){rateRegisteredBoolean=(rate.registered=="true");
if(responseData[item].rateType==rate.rate&&responseData[item].registered==rateRegisteredBoolean){if(responseData[item].term==undefined){if(type=="effective"){ret=effectiveDateFormat(responseData[item].interestRates[0].effectiveDate)
}else{ret=responseData[item].interestRates[0].rateValue
}}else{if(responseData[item].term.value==termArray[0]&&responseData[item].term.type==termArray[1]){if(type=="effective"){ret=effectiveDateFormat(responseData[item].interestRates[0].effectiveDate)
}else{ret=responseData[item].interestRates[0].rateValue
}}}}}else{if(rate.lob=="7"&&dataSimpliiLobName[rate.lob]==responseData[item].rateProductCategory){rateLockedInBoolean=(rate.lockedIn=="true");
if(responseData[item].rateType==rate.rate){if(responseData[item].lockedIn==rateLockedInBoolean){if(type=="effective"){ret=effectiveDateFormat(responseData[item].interestRates[0].effectiveDate)
}else{ret=responseData[item].interestRates[0].rateValue
}}}}else{if(rate.lob=="8"&&dataSimpliiLobName[rate.lob]==responseData[item].rateProductCategory){rateSecuredBoolean=(rate.secured=="true");
if(responseData[item].rateType==rate.rate){if(responseData[item].secured==rateSecuredBoolean){if(type=="effective"){ret=effectiveDateFormat(responseData[item].interestRates[0].effectiveDate)
}else{ret=responseData[item].interestRates[0].rateValue
}}}}else{if(rate.lob=="9"&&dataSimpliiLobName[rate.lob]==responseData[item].rateProductCategory){if(type=="effective"){ret=effectiveDateFormat(responseData[item].interestRates[0].effectiveDate)
}else{ret=responseData[item].interestRates[0].rateValue
}return ret
}}}}}}}}}}return ret
}function effectiveDateFormat(ret){if(ret){try{ret=new Date(ret)
}catch(e){ret=null
}}return ret
...
}function parseQualifier(qualifier){return{output:qualifier[1],lob:qualifier[2],product:qualifier[3],type:qualifier[4],term:(qualifier[5]!=null?qualifier[5].split(",")[0]:null),balance:(qualifier[5]!=null?qualifier[5].split(",")[1]:null),tierFrom:(qualifier[5]!=null?qualifier[5].split(",")[1]:null),registered:(qualifier[5]!=null?qualifier[5].split(",")[3].split("-")[0]:null),secured:(qualifier[5]!=null?qualifier[5].split(",")[3].split("-")[1]:null),investment:(qualifier[5]!=null?qualifier[5].split(",")[3].split("-")[2]:null),taxFree:(qualifier[5]!=null?qualifier[5].split(",")[3].split("-")[3]:null),lockedIn:(qualifier[5]!=null?qualifier[5].split(",")[3].split("-")[4]:null),accountAge:(qualifier[5]!=null?qualifier[5].split(",")[3].split("-")[5]:null),rate:(qualifier[5]!=null?qualifier[5].split(",")[2]:null),customer:(qualifier[5]!=null?qualifier[5].split(",")[3]:null),format:qualifier[7]}
...
for(var i=0;
i<rates.length;
i++){data=getValue(rates[i],rates[i].output);
...
configuration._system.resolver=$.getScript("https://www.cibc.com/applyforproduct/sysdate",function(){configuration._system.date=(systemDate?new Date(systemDate):null);
for(var i=0;
...
}window.services.RDS.requestRates(configuration._data,{success:function(data,lob){dataset=data;
updateElements(lob)
}});
window.services.RDS.updateSandPRDSBannerAdsRates=function(){getSandPRDSBannerAds();
window.services.RDS.requestRates(configuration._data,{success:function(data,lob){updateElements(lob)
}})
};
window.services.RDS.updatePageRates=function(){getModalRDSSpans();
window.services.RDS.requestRates(configuration._data,{success:function(data,lob){updateElements(lob)
}})
}
}}});
(function(a){$(function(){$(".flex-menu").each(function(b,g){var c=$(g);
var e=$(".menu-content",c);
var d=c.attr("data-path");
var f=false;
function h(i){if($(i).hasClass("expanded")&&Foundation.utils.is_small_only()){$(i).removeClass("expanded").addClass("collapsed")
}else{$(i).siblings().removeClass("expanded").addClass("collapsed");
$(i).removeClass("collapsed").addClass("expanded")
}e.empty().append($(".item-content",i).clone());
$(document).foundation("equalizer","reflow")
}$(".menu-item",c).each(function(j,k){$(".item-trigger",k).on("click",function(){h(k)
});
if(d.lastIndexOf($(k).attr("data-path"),0)===0){f=true;
h(k)
}if(!f&&$(k).attr("data-default")=="true"){h(k)
}})
})
})
})(window);
(function(a){a(function(){a("header").each(function(j,m){var i=a(m);
var g={target:a(".header-top-bar",m),height:{trigger:a(".header-top-bar",m).height(),min:75,max:75,mobileMax:60},padding:{min:15,max:20}};
var e={target:a(".header-top-bar .logo img",m),height:{min:40,max:60}};
var k={target:a(".header-top-bar > .flex-grid",m)};
var f={target:a(".header-top-bar > .flex-grid > .flex-column",m)};
var c={target:a(".container",g.target),padding:{min:0,max:10}};
var l={target:a("nav")};
var h=0;
var d=a(".flex-dropdown",g.target);
if(d.length){d.each(function(o,n){var p=a(n);
p.on("open",function(){i.addClass("with-dropdown")
}).on("close",function(){i.removeClass("with-dropdown")
})
})
}var b={updateTopbarHeight:function(){if(a(document).scrollTop()>=g.height.trigger){if(!i.hasClass("shrinking")&&!i.hasClass("collapsed")){i.addClass("shrinking");
g.target.stop(true).animate({height:g.height.min+"px"},function(){i.addClass("collapsed").removeClass("expanded").removeClass("shrinking")
});
if(k.target.length&&f.target.length){if(Foundation.utils.is_medium_up()){k.target.stop(true).animate({height:g.height.min+"px"});
f.target.stop(true).animate({height:g.height.min+"px"})
}else{k.target.stop(true).animate({height:g.height.mobileMax+"px"});
f.target.stop(true).animate({height:g.height.mobileMax+"px"})
}}else{g.target.animate({paddingTop:g.padding.min+"px",paddingBottom:g.padding.min+"px"})
}e.target.stop(true).animate({height:e.height.min+"px"});
c.target.stop(true).animate({paddingTop:c.padding.min+"px"})
}}else{if(!i.hasClass("growing")&&!i.hasClass("expanded")){i.addClass("growing");
g.target.stop(true).animate({height:g.height.max+"px"},function(){i.addClass("expanded").removeClass("collapsed").removeClass("growing")
});
if(k.target.length&&f.target.length){if(Foundation.utils.is_medium_up()){k.target.stop(true).animate({height:g.height.max+"px"});
f.target.stop(true).animate({height:g.height.max+"px"})
}else{k.target.stop(true).animate({height:g.height.mobileMax+"px"});
f.target.stop(true).animate({height:g.height.mobileMax+"px"})
}}else{g.target.animate({paddingTop:g.padding.max+"px",paddingBottom:g.padding.max+"px"})
}e.target.stop(true).animate({height:e.height.max+"px"});
c.target.stop(true).animate({paddingTop:c.padding.max+"px"})
}}},updateHeaderPosition:function(){var n=l.target.position();
var o=a(document).scrollTop();
if(n){if(o+g.height.min>=n.top){if(h>o){i.removeClass("header-hidden").addClass("header-visible");
l.target.removeClass("sticky-pinned").addClass("sticky-under-header")
}else{i.removeClass("header-visible").addClass("header-hidden");
l.target.removeClass("sticky-under-header").addClass("sticky-pinned")
}h=o
}else{i.removeClass("header-hidden").addClass("header-visible");
l.target.removeClass("sticky-under-header")
}}}};
b.updateTopbarHeight();
if(a(document).scrollTop()>0){b.updateHeaderPosition()
}a(document).scroll(function(){b.updateTopbarHeight();
b.updateHeaderPosition()
});
a(document).on("keydown",function(n){if(n.which===9&&!a("body.keyboard-tabbing")[0]){a("body").addClass("keyboard-tabbing");
a(document).on("mouseover",function(){a("body").removeClass("keyboard-tabbing")
})
}})
})
})
})(window.jQuery);
$(function(){$(".flex-header > .header-top-bar > .row > .flex-column:first-child > .flex-dropdown > button").attr("id","blq-local-nav");
$(".flex-header > .header-top-bar > .row > .flex-column:last-child > .flex-dropdown:last-child > button").attr("id","sign-on");
$(".cibc-acc-links a").on("click touchstart",function(d){var c=$($(this).attr("href"),document);
var b=c.parent().find(".sign-on");
var a=c.parent().find(".flex-menu");
if(b.length){c.focus();
setTimeout(function(){c.trigger("click");
if($(".saved-cards:visible",b).length){$(".saved-cards select",b).first().focus()
}else{$(".card-number input",b).first().focus()
}},600)
}else{if(a.length){setTimeout(function(){c.trigger("click");
$(".menu-item",a).first().find("button").focus()
},300)
}else{c.focus()
}}})
});
(function(a){$(function(){$('.column[data-equalize="true"], .columns[data-equalize="true"]').each(function(){var c=$(this);
var b=c.parent().attr("data-equalizer")||"";
if(b.length){c.attr("data-equalizer-watch",b)
}})
})
})(window);
...
(function(a){$(function(){var e=$(".flex-dropdown");
function d(){e.each(function(){if($(this).hasClass("expanded")){f($(this))
}})
}function c(h){d();
h.removeClass("collapsed").addClass("expanded").trigger("open");
var g=$("> button.dropdown-button, .search-header-desktop-container-inner button.search-button:first",h);
g.attr("aria-expanded","true");
$(".dropdown-content",h).attr("aria-hidden","false");
$(document).foundation("equalizer","reflow");
setTimeout(function(){g.attr("aria-label",g.find(".close").attr("data-label"))
},300);
$("#close-dropdown").removeClass("collapsed").addClass("expanded").trigger("open");
if($(".header-top-bar .flex-dropdown.expanded").hasClass("theme-dark")){$(".header-top-bar .flex-dropdown.expanded .dropdown-button .icon").css("color","#FFFFFF");
$(".header-top-bar .flex-dropdown.expanded .dropdown-button").css("color","#FFFFFF")
}else{$(".header-top-bar .flex-dropdown.expanded .dropdown-button .icon").css("color","#666666");
$(".header-top-bar .flex-dropdown.expanded .dropdown-button").css("color","#666666")
}}function f(h){h.removeClass("expanded").addClass("collapsed").trigger("close");
var g=$("> button.dropdown-button, .search-header-desktop-container-inner button.search-button:first",h);
g.removeAttr("aria-expanded");
g.attr("aria-label",g.find(".open").attr("data-label"));
$(".dropdown-content",h).attr("aria-hidden","true");
g.focus();
...
}function b(g){if(Foundation.utils.is_small_only()){$(".dropdown-content",g).css({height:"100%",width:"100%"})
}else{$(".dropdown-content",g).css({height:"auto",width:"100%"})
}}e.each(function(){var g=$(this);
$(".dropdown-button, .search-header-desktop-container-inner button.search-button:first",g).click(function(h){h.preventDefault();
if(g.hasClass("bottom-right")){b(g)
}if(g.hasClass("expanded")){f(g)
}else{c(g)
}});
$(".cibc-signon",g).parent().attr("role","dialog")
});
$(document).keydown(function(g){if(g.which==27){d()
}}).click(function(g){if(!$(g.target).closest(".flex-dropdown").length){d()
}});
$("#close-dropdown").click(function(g){d()
})
})
})();